...
static void dump_shader_bytes(const char *tag, const void *data) {
...
extern void *mmap_listener(void *arg) {
  ...
  for (;;) {
    ...
    case GEM_ALLOCATION:
      uint64_t size = c->p2;
      assert(gem_slots.host_address + size <
             data_region_actual_address + DATA_SIZE);
    ...
    case X11_PRESENT:
      ...
      //           tmp_buf[c->p2].bo, tmp_buf[c->p2].bo_fd);
      ...
      if (frame_count % 5000 == 0) {
        fprintf(stderr,
                "------------------SG STATS-----------------------------\n");
        fprintf(stderr,
                "Frame: %lu; MMAPs: %lu; IOCTLs: %lu; Frame latency: %f; "
                "VMEXITS: %ld\n",
                frame_count, mmap_freq, ioctl_freq,
                (double)frame_latency / 5000.0,
                syscall(sys_sg_vmexits_printreset));
        fprintf(stderr,
                "------------------SG STATS-----------------------------\n");
        frame_latency = 0;
      }
